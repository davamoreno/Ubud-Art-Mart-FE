      ...
      <div v-if="stores.length > 0" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-6">
        ...
        >
          ...
          <div class="flex gap-2 pt-3 mt-2 border-t opacity-0 group-hover:opacity-100 transition-opacity">
            <NuxtLink :to="`/admin/main/shop/${toko.slug}`" class="flex-1 text-center px-3 py-1 bg-blue-500 text-white rounded text-xs hover:bg-blue-600">Edit</NuxtLink>
            <button @click="confirmDelete(toko)" class="flex-1 text-center px-3 py-1 bg-red-500 text-white rounded text-xs hover:bg-red-600">Hapus</button>
          </div>
        </div>
      </div>

      <div v-else class="flex flex-col items-center justify-center h-[500px] text-center">
        <svg class="w-24 h-24 text-gray-300 mb-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 14v3m4-3v3m4-3v3M3 21h18M3 10h18M3 7l9-4 9 4M4 10h16v11H4V10z"></path></svg>
