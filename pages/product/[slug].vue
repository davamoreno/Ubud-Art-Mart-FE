...
console.log('Raw Product:', produkStore.product)
await useAsyncData(
  `product-${slug}`,
  () => produkStore.fetchProduct(slug)
);

const product = computed(() => produkStore.product);
console.log('Computed Product:', product)

let showModal = false;
let activeTab = 'deskripsi';
let rating = 5;
let reviewText = '';
...
<template>
  ...
  <section class="px-6 py-8 dark:bg-gray-900 transition-colors duration-300">
    <div
      class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl overflow-hidden flex flex-col md:flex-row gap-8 p-6 max-w-6xl mx-auto">
      <!-- Gambar Produk -->
      <div class="md:w-1/2 flex justify-center items-center">
        <img src="/assets/images/seni_ubud.png" alt="Lukisan Bali"
          class="w-[420px] h-[420px] object-cover rounded-xl" />
      </div>

      <!-- Informasi Produk -->
      <div class="md:w-1/2 flex flex-col justify-between">
        <div>
          <h2 class="text-3xl font-bold text-gray-900 dark:text-white mb-2">{{ product?.title }}</h2>
          <!-- Rating -->
          ...
          <p class="text-base text-gray-500 dark:text-gray-400 mb-4">{{ product?.toko.nama }}</p>

          <!-- Filter Chips -->
          <div class="flex gap-3 mb-6">
            <div>
              <span
                class="px-4 py-1 rounded-full border text-gray-500 text-sm bg-white cursor-pointer hover:bg-gray-100">
              </span>
            </div>
          ...
          <div class="mb-3">
            <div class="inline-flex gap-6 border-b border-gray-300 dark:border-gray-600">
              <span
                ...
                class="text-lg pb-2 cursor-pointer" @click="activeTab = 'detail'">
                Detail
              </span>
              <span
                ...
                class="text-lg pb-2 cursor-pointer" @click="activeTab = 'deskripsi'">
                Deskripsi
              </span>
            </div>
          </div>
          ...
          <div v-if="activeTab === 'deskripsi'">
            <p class="text-base text-gray-700 dark:text-gray-300 leading-relaxed">
              Karya seni otentik yang menggambarkan keindahan alam, budaya, dan spiritualitas Bali dalam goresan penuh
              warna dan makna. Cocok untuk dekorasi bernuansa etnik dan artistik.
            </p>
          </div>
        ...
        <div class="mt-6 flex gap-4">
          <button
            class="bg-[#2F8F60] text-white text-base font-semibold px-6 py-3 rounded-full flex items-center hover:bg-[#276c4f] transition">
            <img src="/assets/images/whatsapp.svg" alt="WhatsApp" class="w-5 h-5 mr-2" />
            Pesan Sekarang
          </button>

          <button
            class="border border-[#2F8F60] text-[#2F8F60] text-base font-semibold px-6 py-3 rounded-full hover:bg-[#f0f9f4] transition"
    ...
    <div class="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
      ...
      <div class="mb-3">
        <i v-for="n in 5" :key="n" class="bi"
          :class="rating >= n ? 'bi-star-fill text-yellow-400' : 'bi-star text-gray-400'"
          style="font-size: 1.5rem; cursor: pointer;" @click="rating = n"></i>
      </div>
      <div class="mb-4">
        <textarea v-model="reviewText"
          class="w-full bg-white border border-gray-300 rounded-xl p-4 resize-none text-gray-700 focus:outline-none focus:ring-2 focus:ring-green-500"
          style="min-height: 300px;" placeholder="Berikan ulasan anda..."></textarea>
      </div>
      <button @click="submitReview"
        class="w-full bg-green-600 hover:bg-green-700 text-white py-2 rounded-lg font-semibold">Kirim</button>
    </div>
  </div>
  ...
  <section class="px-6 py-10 dark:bg-gray-900 flex justify-center">
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 max-w-5xl w-full space-y-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div
          class="md:col-span-2 flex flex-col md:flex-row items-start md:items-center justify-between space-y-4 md:space-y-0">
          ...
          <div class="flex flex-col space-y-2 w-full md:w-2/3">
            <div class="flex items-center gap-3" v-for="n in [5, 4, 3, 2, 1]" :key="n">
              ...
              <div class="w-full bg-gray-200 h-2 rounded-full overflow-hidden">
                <div class="bg-yellow-400 h-2 rounded-full" :style="{ width: n === 5 ? '100%' : '0%' }"></div>
              </div>
            </div>
          </div>
